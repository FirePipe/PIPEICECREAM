...
const SalesStatistics = lazy(() => import("./SalesStatistics").then(m => ({ default: m.SalesStatistics })));
const D3SalesCharts = lazy(() => import("./D3SalesCharts").then(m => ({ default: m.D3SalesCharts })));
const SettingsPanel = lazy(() => import("./SettingsPanel").then(m => ({ default: m.SettingsPanel })));
const CloudStatsView = lazy(() => import("./CloudStatsView").then(m => ({ default: m.CloudStatsView })));
const CatalogPreviewContent = lazy(() => import("./CatalogPreviewContent").then(m => ({ default: m.CatalogPreviewContent })));
const AdminAuditoriaTab = lazy(() => import("./AdminAuditoriaTab").then(m => ({ default: m.AdminAuditoriaTab })));
const AdminInventoryMovementsTab = lazy(() => import("./AdminInventoryMovementsTab").then(m => ({ default: m.AdminInventoryMovementsTab })));
const SalesCharts = lazy(() => import("./SalesCharts").then(m => ({ default: m.SalesCharts })));
...
interface AdminPanelProps {
  ...
  onUpdateProducts: (newProducts: Product[], actionType?: string) => void;
...
type AdminTab = "inventario" | "ventas" | "ajustes" | "estadisticas" | "auditoria" | "mantenimiento" | "movimientos";
...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [hideOutOfStock, setHideOutOfStock] = useState(!!shopConfig?.hideOutOfStock);
  ...
  const testSupabaseConnection = async (isManual = false) => {
    ...
    try {
      const res = await fetch("/api/supabase-status", { signal: controller.signal });
      ...
      if (!contentType || !contentType.includes("application/json")) {
        ...
        console.warn("Respuesta no-JSON recibida:", bodyText.slice(0, 200));
      ...
      const isNetworkErr = err.name === "AbortError" || errMsg.includes("Failed to fetch") || errMsg.includes("fetch");
      if (isNetworkErr) {
        console.warn("[Supabase Diagnóstico] El servidor local se está iniciando o reiniciando:", errMsg);
        errMsg = "El servidor local se está iniciando o reiniciando (Intente de nuevo en unos segundos)";
      } else {
        console.warn("Error al diagnosticar conexión Supabase:", err);
      }
      setSupabaseStatus({
        configured: false,
        connected: false,
  ...
  useEffect(() => {
    ...
    setHideOutOfStock(!!shopConfig.hideOutOfStock);
  ...
  const triggerAutoStatusCheck = (updatedProducts: Product[], actionType?: string) => {
    onUpdateProducts(updatedProducts, actionType);
  };
  ...
  const handleResetSaborizacion = () => {
    setConfirmModal({
      ...
      onConfirm: () => {
        ...
        triggerAutoStatusCheck(zeroed, "reset_inventario");
        setConfirmModal((prev) => ({ ...prev, isOpen: false }));
        setAlertModal({
          isOpen: true,
  ...
  const handleClearSales = () => {
    setConfirmModal({
      isOpen: true,
      title: "¿Vaciar Historial de Ventas?",
      message: "¿Estás seguro de que deseas vaciar todo el historial de ventas? Esta acción es irreversible y se borrará también en la base de datos.",
  ...
  const approvedSales = useMemo(() => {
    let base = sales.filter((s) => s && (s.estado === "Aprobado" || s.estado === "Pre-Aprobado" || s.estado === "Entregado"));
    ...
    if (dashboardTimeRange !== "todos") {
      ...
      if (dashboardTimeRange === "hoy") {
        base = base.filter(s => {
          ...
          return d ? toLocalDateString(d) === todayStr : false;
        });
  ...
  const { totalRecaudado, totalCostoCompra, gananciaNeta, totalDespachados, bestSeller, worstSeller, flavorSalesArray } = useMemo(() => {
    try {
        const totalRecaudado = approvedSales.reduce((acc, curr) => acc + (typeof curr.total === "number" && !isNaN(curr.total) ? curr.total : 0), 0);
        ...
        const totalCostoCompra = approvedSales.reduce(
            (acc, s) => acc + (s.items || []).reduce((sum, item) => {
                ...
                const costo = typeof product?.costo === "number" ? product.costo : (typeof item.costoUnitario === "number" ? item.costoUnitario : (item.nombre.toLowerCase().includes("mango biche") ? 920 : 1140));
        ...
        const totalDespachados = approvedSales.reduce(
            (acc, s) => acc + (s.items || []).reduce((sum, item) => sum + (typeof item.cantidad === "number" && !isNaN(item.cantidad) ? item.cantidad : 0), 0),
            0
        );
        ...
        approvedSales.forEach((s) => {
            (s.items || []).forEach((item) => {
                const qty = typeof item.cantidad === "number" && !isNaN(item.cantidad) ? item.cantidad : 0;
                flavorSalesCount[item.nombre] = (flavorSalesCount[item.nombre] || 0) + qty;
            });
        });
  ...
  approvedSales.forEach((s) => {
    (s.items || []).forEach((item) => {
      const qty = typeof item.cantidad === "number" && !isNaN(item.cantidad) ? item.cantidad : 0;
      const pu = typeof item.precioUnitario === "number" && !isNaN(item.precioUnitario) ? item.precioUnitario : 0;
      flavorRevenue[item.nombre] = (flavorRevenue[item.nombre] || 0) + qty * pu;
    });
  });
  ...
  approvedSales.forEach((s) => {
    ...
    const d = dateObj ? toLocalDateString(dateObj) : s.fecha;
    if (!salesByDateMap[d]) {
    ...
    salesByDateMap[d].Recaudo += safeTotal;
    salesByDateMap[d].Pedidos += 1;
    const qty = (s.items || []).reduce((acc, curr) => acc + (typeof curr.cantidad === "number" && !isNaN(curr.cantidad) ? curr.cantidad : 0), 0);
    salesByDateMap[d].Unidades += qty;
    
    const cost = (s.items || []).reduce((sum, item) => {
      const cu = typeof item.costoUnitario === "number" && !isNaN(item.costoUnitario) ? item.costoUnitario : (item.nombre.toLowerCase().includes("mango biche") ? 920 : 1140);
      const q = typeof item.cantidad === "number" && !isNaN(item.cantidad) ? item.cantidad : 0;
      return sum + cu * q;
    }, 0);
    salesByDateMap[d].Ganancia += (safeTotal - cost);
  });
    ...
    <div className="admin-container w-full max-w-[1600px] mx-auto py-4 sm:py-8 px-2 sm:px-6 animate-fade-in">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <aside className="w-full lg:w-64 shrink-0 space-y-4">
          <div className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-2xl p-2 lg:p-4 shadow-sm flex flex-row lg:flex-col items-center lg:items-stretch gap-2 lg:gap-5 overflow-x-auto lg:overflow-visible no-scrollbar animate-fade-in">
            ...
            {/* Sección: Historial de Movimientos de Inventario */}
            <div className="flex-1 lg:flex-none space-y-1.5 pt-0 lg:pt-1 border-l lg:border-l-0 lg:border-t border-slate-100/60 dark:border-zinc-800/40 pl-4 lg:pl-0 min-w-[140px] lg:min-w-0">
              <span className="hidden lg:block text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-zinc-500 px-3 mb-1 mt-2 font-mono">
                📈 MOVIMIENTOS
              </span>
                      ...
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button
                            ...
                            className="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg dark:text-indigo-400 dark:hover:bg-zinc-800 transition"
                            title="Historial de Stock (Entradas y Salidas)"
                          >
              ...
              <AnimatePresence mode="popLayout">
                {sortedAdminProducts.map((p) => (
                  <motion.div
                    ...
                    transition={{ duration: 0.2 }}
                    className="bg-slate-50 dark:bg-zinc-950/20 p-4 rounded-2xl border border-slate-100 dark:border-zinc-800 space-y-3"
                  >
                   <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-zinc-800/60">
                    ...
                    <div className="flex items-center gap-2">
                      {(shopConfig.catalogSortOrder === "manual" || !shopConfig.catalogSortOrder) && (
                       ...
                       <button
                        onClick={() => setStockHistoryProduct(p)}
                        className="p-2 rounded-lg bg-indigo-50 text-indigo-600 dark:bg-indigo-950/40 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900/30"
      ...
      {activeTab === "estadisticas" && (
        ...
        >
          ...
          {estadisticasView === "cloud" && (
            <Suspense fallback={<CloudStatsSkeleton />}>
              <CloudStatsView key={refreshKey} contrasenaAdmin={contrasenaAdmin} />
            </Suspense>
          )}
          {estadisticasView === "graficas" && (
      ...
      {activeTab === "ajustes" && (
        <Suspense fallback={<SettingsPanelSkeleton />}>
          ...
          setContrasenaAdmin={setContrasenaAdmin}
          tiendaNombre={tiendaNombre}
          setTiendaNombre={setTiendaNombre}
          metodoOrdenar={metodoOrdenar}
          setMetodoOrdenar={setMetodoOrdenar}
          ...
          setWhatsappNumero={setWhatsappNumero}
          ...
          setCatalogModeEnabled={setCatalogModeEnabled}
          catalogModeMessage={catalogModeMessage}
          setCatalogModeMessage={setCatalogModeMessage}
          ...
          onForceSync={async () => {
            ...
            try {
              ...
              setAlertModal({
                ...
                message: "La base de datos local y la nube se han sincronizado correctamente."
              });
            } catch (err) {
              setSyncResult("error");
              setAlertModal({
                ...
                message: "Ocurrió un error al forzar la sincronización."
              });
            } finally {
              setIsSyncing(false);
            }
          ...
          onResetSales={() => {
            setConfirmModal({
              isOpen: true,
              title: "¿Vaciar Historial de Ventas?",
              message: "Se eliminarán permanentemente todas las ventas de la base de datos. ¿Está seguro de vaciar el historial?",
              confirmText: "Vaciar Historial",
              confirmVariant: "danger",
              onConfirm: async () => {
                try {
          ...
          onClearCache={() => {
            setConfirmModal({
              ...
              message: "¿Deseas borrar el almacenamiento local y recargar la aplicación? Esta acción reiniciará tu sesión actual.",
          ...
          <div className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-2xl p-6 max-w-sm w-full shadow-2xl space-y-4">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button
                ...
                className={`px-4 py-2 text-xs font-bold uppercase tracking-wider text-white rounded-xl cursor-pointer transition-all active:scale-95 ${
