...
export async function supabaseFetchShopConfig(): Promise<ShopConfig | null> {
  ...
  try {
    let { data, error } = await client
      .from("shop_config")
      .select("tienda_nombre, contrasena_admin, whatsapp_telefono, metodo_ordenar, cuenta_numero, cuenta_titular, mostrar_reloj, mostrar_clima, sync_enabled, catalog_sort_order, catalog_mode_enabled, catalog_mode_message, hide_out_of_stock, has_seen_tutorial")
      .eq("id", "primary")
      .single();

    if (error && (error.message?.includes("column") || error.code === "PGRST204")) {
      console.warn("[Supabase Engine] Newer columns missing in shop_config, falling back to legacy select query...");
...
export async function supabaseSaveShopConfig(config: ShopConfig): Promise<boolean> {
  ...
  try {
    const payload: any = {
      ...
      hide_out_of_stock: !!config.hideOutOfStock,
    ...
    if (error && (error.message?.includes("column") || error.code === "PGRST204")) {
      console.warn("[Supabase Engine] Column missing in remote shop_config, attempting fallback save without new columns...");
      delete payload.hide_out_of_stock;
      delete payload.has_seen_tutorial;
      const retryResult = await client
        .from("shop_config")
        .upsert(payload, { onConflict: "id" });
      error = retryResult.error;
