...
import { getFullDb, saveProductsToDb, saveSalesToDb, saveConfigToDb, restoreDb, saveAuditLogToDb, getAuditLogsFromDb, deleteAuditLogFromDb, clearAuditLogsFromDb, optimizeAuditLogsInDb, getPaginatedAuditLogsFromDb, getInventoryMovementsFromDb, clearInventoryMovementsInDb, recordInventoryMovementInDb } from "./db";
...
app.post("/api/checkout", async (req, res) => {
  ...
  try {
    ...
    // Record inventory movements for sold products
    for (const item of sale.items) {
      const dbProduct = data.products.find((p: any) => p.id === item.productId);
      const prevStock = dbProduct ? dbProduct.stock : 0;
      const newStock = Math.max(0, prevStock - item.cantidad);
      await recordInventoryMovementInDb({
        product_id: item.productId,
        product_nombre: item.nombre || (dbProduct ? dbProduct.nombre : "Producto"),
        tipo_movimiento: "venta",
        cantidad_cambio: -item.cantidad,
        stock_anterior: prevStock,
        stock_nuevo: newStock,
        motivo: `Venta realizada en Orden ${orderId}`,
        responsable: `Cliente: ${sale.clienteNombre || "Cliente"}`
      });
    }
...
app.post("/api/db/products", adminAuthMiddleware, async (req, res) => {
  try {
    ...
    const actionType = req.headers["x-action-type"] as string;

    if (actionType === "reset_inventario") {
      await saveAuditLogToDb({
        fecha: now.toISOString().split("T")[0],
        hora: now.toTimeString().split(" ")[0].substring(0, 5),
        accion: "reset_inventario",
        admin_responsable: adminName,
        entidad_afectada: "products_all",
        entidad_id: "all",
        valor_anterior: { descripcion: `Stock anterior de ${oldProducts.length} productos` },
        valor_nuevo: { descripcion: "Todos los sabores reseteados a stock 0" },
        ip,
        navegador: uaInfo.browser,
        dispositivo: uaInfo.device
      });
      ...
      for (const oldP of oldProducts) {
        if (oldP.stock > 0) {
          await recordInventoryMovementInDb({
            product_id: oldP.id,
            product_nombre: oldP.nombre,
            tipo_movimiento: "ajuste_manual",
            cantidad_cambio: -oldP.stock,
            stock_anterior: oldP.stock,
            stock_nuevo: 0,
            motivo: `Reinicio masivo de inventario a 0 por ${adminName}`,
      ...
      for (const newP of result.data.products) {
        ...
        if (!oldP) {
          ...
          await recordInventoryMovementInDb({
            product_id: newP.id,
            product_nombre: newP.nombre,
            tipo_movimiento: "creacion_producto",
            cantidad_cambio: newP.stock || 0,
            stock_anterior: 0,
            stock_nuevo: newP.stock || 0,
            motivo: `Creación de nuevo sabor por ${adminName}`,
            responsable: adminName
          });
          ...
          if (oldP.stock !== newP.stock) {
            action = "cambio_stock";
            const diff = newP.stock - oldP.stock;
            await recordInventoryMovementInDb({
              product_id: newP.id,
              product_nombre: newP.nombre,
              tipo_movimiento: diff > 0 ? "reposicion" : "ajuste_manual",
              cantidad_cambio: diff,
              stock_anterior: oldP.stock,
              stock_nuevo: newP.stock,
              motivo: `Ajuste de stock por ${adminName}`,
              responsable: adminName
            });
...
// POST mark tutorial as seen in shopConfig (public route for onboarding completion)
app.post("/api/db/tutorial-seen", async (_req, res) => {
  try {
    const fullDb = await getFullDb();
    const currentConfig: any = { ...(fullDb.shopConfig || {}) };
    currentConfig.hasSeenTutorial = true;
    await saveConfigToDb(currentConfig as ShopConfig);
    res.json({ success: true, hasSeenTutorial: true });
  } catch (error: any) {
    console.error("[Tutorial] Error saving tutorial preference:", error);
    res.status(500).json({ error: "Failed to update tutorial state", message: error.message });
  }
});
...
// ==========================================
app.get("/api/admin/inventory-movements", adminAuthMiddleware, async (req, res) => {
  try {
    const page = Number(req.query.page) || 1;
    const limit = Number(req.query.limit) || 20;
    const tipo = (req.query.tipo as string) || "";
    const sort = (req.query.sort as string) || "reciente";

    const result = await getInventoryMovementsFromDb({ page, limit, tipo, sort });
    res.json(result);
  } catch (err: any) {
    console.error("[API] Error fetching inventory movements:", err);
    res.status(500).json({ error: "Failed to fetch inventory movements", message: err.message });
  }
});

app.post("/api/admin/inventory-movements/clear", adminAuthMiddleware, async (req, res) => {
  try {
    const success = await clearInventoryMovementsInDb();
    res.json({ success });
  } catch (err: any) {
    console.error("[API] Error clearing inventory movements:", err);
    res.status(500).json({ error: "Failed to clear inventory movements", message: err.message });
  }
});
...
// AI Ice Cream Recommendation of the Day
app.get("/api/ai/helado-del-dia", async (req, res) => {
  try {
    ...
    const dbData = await getFullDb(true);
    const availableProducts = (dbData.products || []).filter((p: any) => p.stock > 0);
    
    if (availableProducts.length === 0) {
      return res.json({
        success: true,
        recommendation: "¡Hoy todos los sabores son excepcionales! Pregunta por tu favorito. ✨"
      });
    }

    // Dynamic helper to choose a random product name for high variability
    const getRandomProduct = () => {
      const p = availableProducts[Math.floor(Math.random() * availableProducts.length)];
      return p ? p.nombre : "un helado delicioso";
    };
    
    // Clean fallback list of fun fortune templates with lazy, randomized evaluation
    const fallbackTemplates = [
      () => `¡Un antojo de ${getRandomProduct()} es justo lo que tu día necesita hoy! 🍦`,
      () => `El destino dice: es el momento perfecto para disfrutar un helado de ${getRandomProduct()}. ✨`,
      () => `¿Día difícil? Nada que un delicioso helado de ${getRandomProduct()} no pueda curar. 🥠`,
      () => `Hoy la fortuna te sonríe con un sabor inigualable: ¡ve por tu ${getRandomProduct()}! 💫`,
      () => `La frescura hecha helado: ¡hoy déjate sorprender por el sabor ${getRandomProduct()}! 🧊`,
      () => `¡Hoy es tu día de suerte! Disfruta la cremosidad y el sabor de nuestro delicioso ${getRandomProduct()}. 🌟`,
      () => `Un clásico que nunca falla: hoy déjate llevar por el sabor ${getRandomProduct()}. 🍧`,
      () => `Endulza tu día con el toque mágico de nuestro sabor ${getRandomProduct()}. ¡Te lo mereces! 🍯`,
      () => `La felicidad viene en conos, especialmente si es de ${getRandomProduct()}. ¡Próbalo hoy! 🍨`
    ];
    
    // Choose a random template for default fallback and evaluate it
    const defaultRec = fallbackTemplates[Math.floor(Math.random() * fallbackTemplates.length)]();

    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) {
      console.log("Using local randomized fallback recommendation.");
      return res.json({ success: true, recommendation: defaultRec });
    }

    try {
      const ai = getGeminiClient();
      
      // Shuffle products to give Gemini a randomized context each time
      const shuffledProducts = [...availableProducts].sort(() => 0.5 - Math.random());
      const selectedProducts = shuffledProducts.slice(0, 3);
      const productListText = selectedProducts.map(p => `- ${p.nombre}`).join("\n");
      
      const seed = Math.floor(Math.random() * 10000);
      const prompt = `Actúa como un maestro heladero creativo y experto en marketing. Escribe una sugerencia o recomendación tipo "galleta de la suerte" o "frase del día" ultra corta, inspiradora, fresca, y divertida (máximo 110 caracteres) en español sugiriendo un "Helado del día" basado en uno de los siguientes productos:\n${productListText}\n\nSemilla de aleatoriedad para que sea completamente única: ${seed}.\nLa frase debe ser corta, alegre, invitar a probarlo hoy mismo, y terminar con un emoji adecuado. No utilices palabras aburridas o genéricas. Evita explicaciones o textos introductorios, devuelve SOLO la frase final directa para mostrar al cliente sin comillas externas.`;
      ...
      const recommendation = response.text ? response.text.trim().replace(/^["']|["']$/g, '') : defaultRec;
      res.json({ success: true, recommendation });
    } catch (apiErr: any) {
      console.error("Gemini API Error in helado-del-dia:", apiErr);
      res.json({ success: true, recommendation: defaultRec });
    }
  } catch (error: any) {
    console.error("Error in helado-del-dia endpoint:", error);
