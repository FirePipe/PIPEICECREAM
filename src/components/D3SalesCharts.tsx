...
export const D3SalesCharts: React.FC<D3SalesChartsProps> = ({ sales }) => {
  ...
  const timeSeriesData = useMemo(() => {
    ...
    validSales.forEach(s => {
      // s.fecha is usually YYYY-MM-DD or DD/MM/YYYY. Let's parse it safely.
      // Assuming DD/MM/YYYY based on formatCOP usage and Spanish locale
      let dateObj = new Date();
      if (s.fecha.includes('/')) {
        const [day, month, year] = s.fecha.split('/');
        dateObj = new Date(parseInt(year), parseInt(month) - 1, parseInt(day));
      } else {
        dateObj = new Date(s.fecha);
      }
    ...
    return Array.from(map.entries())
      ...
      // .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()) // Key is already string formatted, so sorting might be tricky, we'll keep insertion order or chronological
      .slice(-14);
