...
import {
  ...
  supabaseFetchInventoryMovements,
  supabaseClearInventoryMovements
} from "./supabase";
...
export async function recordInventoryMovementInDb(movement: {
  product_id: string;
  ...
  const movementRecord = {
    ...
    product_nombre: movement.product_nombre,
    tipo_movimiento: movement.tipo_movimiento,
    cantidad_cambio: movement.cantidad_cambio,
    stock_anterior: movement.stock_anterior,
    stock_nuevo: movement.stock_nuevo,
    motivo: movement.motivo || "",
    responsable: movement.responsable || "sistema",
  };
  ...
  if (isSupabaseConfigured()) {
    try {
      const client = getSupabaseClient();
      if (client) {
        await client.from("inventory_movements").insert(movementRecord);
      }
    } catch (err) {
      console.warn("[Supabase Engine] Non-fatal: error saving movement to Supabase:", err);
    }
  }

  const current = await getFullDb();
  if (!current.inventoryMovements) {
    current.inventoryMovements = [];
  }
  current.inventoryMovements.unshift(movementRecord);
  if (current.inventoryMovements.length > 1000) {
    current.inventoryMovements = current.inventoryMovements.slice(0, 1000);
  }

  try {
    fs.writeFileSync(DB_FILE, JSON.stringify(current, null, 2), "utf-8");
  } catch (err) {
    console.error("Failed writing inventory movement to local db.json:", err);
  }
}

export async function getInventoryMovementsFromDb(options: { page?: number; limit?: number; tipo?: string; sort?: string }): Promise<{ movements: any[]; total: number }> {
  if (isSupabaseConfigured()) {
    try {
      const sRes = await supabaseFetchInventoryMovements(options);
      if (sRes && sRes.movements && sRes.movements.length > 0) {
        return sRes;
      }
    } catch (e) {
      console.warn("[Database Engine] Could not fetch movements from Supabase, falling back to local:", e);
    }
  }
  ...
  if (options.tipo && options.tipo.trim() !== "") {
    movements = movements.filter((m: any) => m.tipo_movimiento === options.tipo.trim());
  }
  if (options.sort === "antiguo") {
  ...
  return { movements: paginated, total: movements.length };
}

export async function clearInventoryMovementsInDb(): Promise<boolean> {
  if (isSupabaseConfigured()) {
    try {
      await supabaseClearInventoryMovements();
    } catch (err) {
      console.warn("[Database Engine] Error clearing Supabase inventory movements:", err);
    }
  }
  ...
  try {
    ...
    console.error("Failed to clear local inventory movements in db.json:", err);
  }
  return true;
}
