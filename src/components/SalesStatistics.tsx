...
export const SalesStatistics: React.FC<SalesStatisticsProps> = ({ sales }) => {
  ...
  const filteredSales = useMemo(() => {
    const validSales = sales.filter((s) => s && s.estado !== "Eliminada" && s.estado !== "Rechazado");
    const now = new Date();
    ...
    if (period === "currentMonth") {
      ...
      return validSales.filter((s) => {
        const sDate = parseSaleDate(s.fecha);
        return sDate ? (sDate.getFullYear() === currentYear && sDate.getMonth() === currentMonthNum) : false;
      });
    }
  ...
  const trendData = useMemo(() => {
    ...
    filteredSales.forEach((s) => {
      ...
      const current = dailyMap.get(dateStr) || { totalSales: 0, totalCost: 0 };
    ...
    return Array.from(dailyMap.entries())
      .map(([dateStr, data]) => {
        const dateObj = parseSaleDate(dateStr);
