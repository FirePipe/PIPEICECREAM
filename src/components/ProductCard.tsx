...
interface ProductCardProps {
  ...
  loading?: boolean;
  onAddToCart?: (product: Product, cantidad: number, startRect?: DOMRect, imageUrl?: string) => void;
  cartQty?: number;
...
export const ProductCard: React.FC<ProductCardProps> = React.memo(({
  ...
  const handleAddClick = (e: React.MouseEvent) => {
    ...
    // Capture product card element bounding box for Fly-To-Cart parabolic trajectory
    const cardElement = cardRef.current || (e.currentTarget as HTMLElement);
    const imgElement = cardElement.querySelector("img") || cardElement;
    const startRect = imgElement.getBoundingClientRect();

    onAddToCart(product, selectedQty, startRect, product.imagen);
    setIsAdded(true);
    setIsShaking(true);
